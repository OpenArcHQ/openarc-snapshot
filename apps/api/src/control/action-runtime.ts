import { CommerceActionRateLimiter } from "./action-rate-limiter.js";
import { CommerceActionService } from "./action-service.js";
import type {
...
/**
 * Dedicated runtime wiring for the protected commerce-action slice.
 *
 * The server NEVER runs migrations. This runtime owns no pool and constructs no
 * concrete repository: the DB10/DB11 commerce-action store is authored
 * independently and is injected through the narrow `CommerceActionStorePort`
 * seam, together with the commerce-session read, the accepted auth seam, the
 * purpose-separated rate secret and the accepted durable rate-limit store. That
 * keeps this module free of any database import and lets the integrator bind
 * the real store without touching the HTTP slice.
 *
 ...
 * close on a caller-supplied dependency. An enabled action surface is a control
 * surface only: it never implies that a payment, settlement, delivery or grant
 * lane exists.
 *
 ...
 */
export interface CommerceActionStoreLifecyclePort {
  initialize?(): Promise<void>;
  readiness?(): Promise<void>;
  close?(): Promise<void>;
}

export interface CommerceActionRuntimeDependencies {
  /** Default OFF. A false/absent gate installs no service at all. */
  readonly enabled?: boolean;
  readonly authSecret?: string;
  readonly auth?: CommerceActionAuthPort;
  readonly rateLimitStore?: CommerceActionRateLimitStorePort;
  readonly store?: CommerceActionStorePort;
  readonly commerceSessions?: CommerceSessionReadPort;
  readonly lifecycle?: CommerceActionStoreLifecyclePort;
}

export type CommerceActionRuntimeState = "enabled" | "built_disabled";

export interface StartedCommerceActionRuntime {
  readonly state: CommerceActionRuntimeState;
  /** Present ONLY in the `enabled` state. */
  readonly service: CommerceActionService | undefined;
  ready(): Promise<boolean>;
  close(): Promise<void>;
}

function unavailable(): never {
  throw new Error("COMMERCE_ACTION_RUNTIME_UNAVAILABLE");
}

function isAuthPort(value: unknown): value is CommerceActionAuthPort {
  return (
    typeof value === "object" &&
    value !== null &&
...
function isRateLimitStorePort(
  value: unknown,
): value is CommerceActionRateLimitStorePort {
  return (
    typeof value === "object" &&
    value !== null &&
...
/** The exact twelve operations the service depends on. */
...
function isStorePort(value: unknown): value is CommerceActionStorePort {
...
function isCommerceSessionReadPort(
  value: unknown,
): value is CommerceSessionReadPort {
  return (
    typeof value === "object" &&
    value !== null &&
 ...
 */
export function actionDependenciesConfigured(
...
function disabledRuntime(): StartedCommerceActionRuntime {
  return {
...
export async function startCommerceActionRuntime(
  dependencies: CommerceActionRuntimeDependencies,
  ...
  if (!actionDependenciesConfigured(dependencies)) return disabledRuntime();

  const lifecycle = dependencies.lifecycle;
  let service: CommerceActionService;
  try {
    ...
    const limits = new CommerceActionRateLimiter({
      ...
      store: dependencies.rateLimitStore as CommerceActionRateLimitStorePort,
    });
    ...
    service = new CommerceActionService({
      auth: dependencies.auth as CommerceActionAuthPort,
      store: dependencies.store as CommerceActionStorePort,
      commerceSessions: dependencies.commerceSessions as CommerceSessionReadPort,
      limits,
    });
  } catch {
