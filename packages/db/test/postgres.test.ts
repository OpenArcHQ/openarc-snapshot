...
describe('postgres migrations', () => {
  ...
  it('clean install then readSchemaVersion and rerun is idempotent', async () => {
    ...
    try {
      await migrate(pool);
      expect(await readSchemaVersion(pool)).toBe(14);
      await migrate(pool);
      expect(await readSchemaVersion(pool)).toBe(14);
    } finally {
      await pool.end();
    ...
    try {
      ...
      try {
        ...
        const meta = await client.query<{ n: number }>(
          "SELECT count(*)::int AS n FROM openarc_meta.schema_migrations WHERE id = '0015_synthetic'",
        );
      ...
      const fixed: SqlMigration[] = [
        ...base,
        { id: '0015_synthetic', sql: 'CREATE TABLE openarc_auth.synthetic_ok (id int);' },
      ];
  ...
  it('rejects a changed applied ID', async () => {
    ...
    try {
      ...
      const client = await pool.connect();
      try {
        await client.query("UPDATE openarc_meta.schema_migrations SET id = '0015_renamed' WHERE id = '0001_auth'");
      } finally {
        client.release();
      }
      await expectError(migrate(pool, base), 'MIGRATION_DB_MISMATCH');
  ...
  it('applies a successful synthetic upgrade', async () => {
    ...
    try {
      ...
      const upgrade: SqlMigration[] = [
        ...base,
        { id: '0015_upgrade', sql: 'CREATE TABLE openarc_auth.upgrade_ok (id int);' },
      ];
  ...
  it('serializes two concurrent migration callers', async () => {
    ...
    try {
      ...
      expect(await readSchemaVersion(first)).toBe(14);
    } finally {
      await first.end();
      await second.end();
    }
  ...
  it('forbids the runtime role from DDL and migration history writes', async () => {
    ...
    try {
      ...
      expect(await readSchemaVersion(app)).toBe(14);
    } finally {
      await app.end();
    }
  });
