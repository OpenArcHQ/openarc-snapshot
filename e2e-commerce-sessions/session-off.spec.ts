...
// Commerce-sessions flag OFF.
//
// Same parent flags as the ON server (account access, tenant reads and the API
// boundary enabled) but the session flag is false. The shell must make ZERO
// session-capability or session requests, and the pre-existing workspace routes
...
async function instrument(page: Page): Promise<void> {
  await page.addInitScript(() => {
    const w = window as typeof window & { __sessionPaths?: string[] };
    w.__sessionPaths = [];
    const originalFetch = window.fetch.bind(window);
    window.fetch = (input: RequestInfo | URL, init?: RequestInit) => {
      const value = String(input);
      if (
        value.includes("session-capabilities") ||
        value.includes("/v2/control/organizations") ||
        value.includes("/v2/agent/commerce-sessions")
      ) {
        w.__sessionPaths?.push(`${init?.method ?? "GET"} ${value}`);
      }
...
async function expectNoSessionPaths(page: Page): Promise<void> {
  const paths = await page.evaluate(() => (window as typeof window & { __sessionPaths?: string[] }).__sessionPaths ?? []);
  expect(paths).toEqual([]);
}

test("makes zero session requests on the session roots route", async ({ page }) => {
  await instrument(page);
  await stubParents(page);
  await open(page, "/app/sessions");
  await expect(page.getByRole("heading", { name: "This section is not available yet" })).toBeVisible();
  await expectNoSessionPaths(page);
});

test("makes zero session requests on the session new route", async ({ page }) => {
  await instrument(page);
  await stubParents(page);
  await open(page, "/app/sessions/new");
  await expect(page.getByRole("heading", { name: "This section is not available yet" })).toBeVisible();
  await expectNoSessionPaths(page);
});

test("makes zero session requests on a session detail route", async ({ page }) => {
  await instrument(page);
  await stubParents(page);
  await open(page, `/app/sessions/${encodeURIComponent(UUID)}`);
  await expect(page.getByRole("heading", { name: "This section is not available yet" })).toBeVisible();
  await expectNoSessionPaths(page);
});
